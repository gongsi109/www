...
class Buy_1 extends Model {
    ...
    public function getMansongruleCartListByTotal($goods_total) {
        if (!config('promotion_allow') || empty($goods_total) || !is_array($goods_total))
    ...
    public function reCalcGoodsTotal($goods_total, $preferential_array, $preferential_type) {
        $deny = empty($goods_total) || !is_array($goods_total) || empty($preferential_array) || !is_array($preferential_array);
        if ($deny)
            return $goods_total;
     ...
     * @param array $goods_total (店铺ID=>商品总金额)
     * @return array
     */
    public function reParseVoucherList($input_voucher_list = array(), $goods_total = array(), $member_id) {
        ...
        if (is_array($tmp) && isset($tmp[$voucher['vouchertemplate_id']])) {
            $input_voucher_list['voucher_id'] = $tmp[$voucher['vouchertemplate_id']]['voucher_id'];
            $input_voucher_list['voucher_code'] = $tmp[$voucher['vouchertemplate_id']]['voucher_code'];
            $input_voucher_list['voucher_owner_id'] = $tmp[$voucher['vouchertemplate_id']]['voucher_owner_id'];
        } else {
    ...
    public function getPromotionTotal($goods_total, $final_goods_total) {
        if (!is_array($goods_total) || !is_array($final_goods_total))
            return array();
    ...
    public function getFreightDescList($goods_total) {
        return array(array(), array());
        if (empty($goods_total) || !is_array($goods_total))
            return array(array(), array());
